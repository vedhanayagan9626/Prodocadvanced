...
class EnhancedZohoInvoiceParser:
    ...
    def process_invoice(self, text, path="", export_excel=True):
        """Main processing function that returns Zoho-formatted data"""
        try:
            # Extract all data using your existing methods
            items_data = self.extract_items_from_text(text)
            invoice_meta = self.extract_invoice_data(text)
            seller, buyer = self.extract_seller_buyer(text)
            amount = self.extract_amount_and_totals(text)
            taxes = self.extract_taxes(text)

            print("=" * 50)
            print("EXTRACTION SUMMARY:")
            print(f"Items found: {len(items_data)}")
            print(f"Invoice: {invoice_meta.get('invoice_number', 'Unknown')}")
            print(f"Date: {invoice_meta.get('invoice_date', 'Unknown')}")
            print(f"Seller: {seller.get('company_name', 'Unknown')}")
            print(f"Buyer: {buyer.get('company_name', 'Unknown')}")
            print(f"Total: {amount.get('total_amount', '0.00')}")
            print("=" * 50)

            # Map to Zoho format
            zoho_data = self.map_to_zoho_format(invoice_meta, seller, buyer, items_data, amount, taxes)

            # Export to Excel if requested
            if export_excel:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                invoice_num = invoice_meta.get('invoice_number', 'unknown').replace('/', '_')
                excel_filename = f"zoho_import_{invoice_num}_{timestamp}.xlsx"
                
                success = self.create_excel_export(zoho_data, excel_filename)
                if success:
                    print(f"\nExcel file created: {excel_filename}")
                    zoho_data['excel_file'] = excel_filename
                else:
...
def process_invoice(text, path):
    ...
    result = parser.process_invoice(text, path, export_excel=True)
    ...
    if result:
        ...
        print(f"Total Amount: ₹{result['invoice_header']['Total']}")
